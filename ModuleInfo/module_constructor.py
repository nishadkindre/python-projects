...
class ModuleC:
    ...
    def print_output(self, input_: str = 'self.all_info()') -> str:  # default parameter = 'self.all_info()'
        '''Helper Method.
        ...
        Special Method for use in the export_info() method.
        '''
        import io
        import sys
        old = sys.stdout 
...
class Module(ModuleC):
    ...
    def all_info(self) -> None:
        ...
        self.methods_list()
        print(X)
        all_methods_info = tuple(map(lambda a: self.method_info_(a), range(1, len(self.data)+1)))  # if assigned at top executes below statement first.
